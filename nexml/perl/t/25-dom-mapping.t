...
my ($vol,$td, $tf) = File::Spec->splitpath(File::Spec->rel2abs(__FILE__));
$tf = File::Spec->catdir($vol, $td, 'data/01_basic.xml');
ok( -e $tf, 'test data present');
my $twig = XML::Twig->new();
ok( $twig->parsefile($tf), 'test data parsed');
