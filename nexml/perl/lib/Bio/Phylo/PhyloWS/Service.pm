    ...
    sub handle_request {
        ...
        if ( $path_info =~ m|.*/phylows/(.+?)$| ) {
            ...
            if ( my $redirect = $self->get_redirect($cgi) ) {
                 print $cgi->redirect( '-uri' => $redirect, '-status' => _HTTP_SC_SEE_ALSO_, );
            }            
            elsif ( $guid !~ m|/find/?| ) {
                if ( my $format = $cgi->param('format') ) {
                    print $cgi->header($Bio::Phylo::PhyloWS::MIMETYPE{$format});
                    print $self->get_serialization( '-guid' => $guid, '-format' => $format, );
                }
                else {
                ...
                if ( my $format = $cgi->param('format') ) {                    
                    ...
                    print $self->get_description( 
                        '-guid' => 'tree/find?query=' . URI::Escape::uri_escape($query) )->to_xml;
                }            
...
=cut
    ...
    sub get_record {
        my $self = shift;
        throw 'NotImplemented' => 'Method get_record should be in ' . ref($self) . ", but isn't";
    }
...
=cut
    ...
    sub get_query_result {
        my $self = shift;
        throw 'NotImplemented' => 'Method get_query_result should be in ' . ref($self) . ", but isn't";
    }    
...
=cut
    ...
    sub get_supported_formats {
        my $self = shift;
        throw 'NotImplemented' => 'Method get_supported_formats should be in ' . ref($self) . ", but isn't";
    }    
