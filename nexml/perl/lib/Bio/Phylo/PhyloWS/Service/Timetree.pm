...

{
    ...
    my $create_project = sub {
        ...
        my $taxona  = $fac->create_taxon( '-xml_id' => 'taxona', '-name' => 'taxon_a' );
        my $taxonb  = $fac->create_taxon( '-xml_id' => 'taxonb', '-name' => 'taxon_b' );
    ...
    my $create_node = sub {
        ...
        $fac->create_node( 
            ...
            '-parent'        => $root,
            '-taxon'         => $taxon_name,
            '-name'          => $date->{$field}=~m/\s/?"'".$date->{$field}."'":$date->{$field},
        );        
    };
    ...
    sub get_query_result {
        ...
        for my $date ( @{ $dates } ) {
            ...
            for my $field ( qw(genes data source) ) {
                $tree->add_meta(
                    $fac->create_meta( '-triple' => { "tt:$field" => $date->{$field} } )
                );
            }
            $tree->add_meta(
                $fac->create_meta( '-triple' => { "tt:pub" => HTML::Entities::encode_entities($pubmed.$date->{'pub'}.'&dopt=Abstract') } )
            );
