/* xerces-based parser written by Terri Liebowitz Schwartz of SDSC (see CIPRES
license).

Executable takes three arguments:
	path-to-instance-document path-to-xml-schema namespace
...
public class XmlValidator
{
	...
	// This implementation works fine with java 1.5 AND allows us to specify
	// a schema in the xml file but override it here programmatically. However
	// I've now spent days trying to get the right combination of jars to get
	// this
	// to work with java 1.4.2 and I'm pulling my hair out frustrated and giving
	// up
	// for the time being. Back to using the old implementation below.

	/*
	 * public static String againstXSD(String xmlFilepath, String xsdFilepath) {
	 * try { //make sure both paths specify a valid file File config_fp = new
	 * File(xmlFilepath); if (!config_fp.exists()) { return "Unable to open file " +
	 * xmlFilepath; } config_fp = new File(xsdFilepath); if
	 * (!config_fp.exists()) { return "Unable to open file " + xsdFilepath; }
	 * DocumentBuilder parser =
	 * DocumentBuilderFactory.newInstance().newDocumentBuilder(); Document
	 * document = parser.parse(new File(xmlFilepath)); SchemaFactory factory =
	 * SchemaFactory.newInstance(XMLConstants.W3C_XML_SCHEMA_NS_URI); Source
	 * schemaFile = new StreamSource(new File(xsdFilepath)); Schema schema =
	 * factory.newSchema(schemaFile); Validator validator =
	 * schema.newValidator(); validator.validate(new DOMSource(document));
	...
	public static String againstXSD(final String xmlFilepath, final String xsdFilepath, final String namespace)
	{
		...
		try
		{
			...
			{
				parser.setProperty("http://apache.org/xml/properties/schema/external-schemaLocation", namespace.trim()
				                                                                                      + " "
				                                                                                      + xsdFilepath);
			}
