...
=cut
    ...
    sub get_query_result {
        ...
        if ( my %args = looks_like_hash @_ ) {
            my $query = $args{'-query'} || throw 'BadArgs' => "Need query argument";
            ...
            if ( $response->is_success ) {
                my $desc = $fac->create_description( '-url' => $url, '-guid' => 'find' );
...
=cut
    ...
    sub get_record {
        my $self = shift;
        if ( my %args = looks_like_hash @_ ) {
            my $guid = $args{'-guid'} || throw 'BadArgs' => "Need -guid argument";
